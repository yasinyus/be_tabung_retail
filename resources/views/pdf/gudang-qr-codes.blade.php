...
<body>
    <div class="header">
        <h2>QR Codes Gudang</h2>
        <p>Generated: {{ date('d/m/Y H:i') }} | Total: {{ count($qrData) }} gudang</p>
    </div>
            ...
            <tr>
                @foreach($chunk as $data)
                    <td class="qr-cell">
                        ...
                        <div class="info">
                            <div class="kode-gudang">Kode: {{ $data['gudang']->kode_gudang }}</div>
                            <div>Nama: {{ $data['gudang']->nama_gudang }}</div>
                            <div>Alamat: {{ Str::limit($data['gudang']->alamat, 30) }}</div>
                            <div>Kapasitas: {{ $data['gudang']->kapasitas }} unit</div>
                        </div>
                    </td>
                @endforeach
