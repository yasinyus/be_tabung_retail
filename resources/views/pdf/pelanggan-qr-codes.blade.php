...
<head>
    <meta charset="utf-8">
    <title>QR Codes Pelanggan</title>
    <style>
        @page {
            margin: 20px;
        }
...
<body>
    <div class="header">
        <h2>QR Codes Pelanggan</h2>
        <p>Generated: {{ date('d/m/Y H:i') }} | Total: {{ count($qrData) }} pelanggan</p>
    </div>
            ...
            <tr>
                @foreach($chunk as $data)
                    <td class="qr-cell">
                        ...
                        <div class="info">
                            <div class="kode-pelanggan">Kode: {{ $data['pelanggan']->kode_pelanggan }}</div>
                            <div>Nama: {{ $data['pelanggan']->nama_pelanggan }}</div>
                            <div>Alamat: {{ Str::limit($data['pelanggan']->alamat, 30) }}</div>
                            <div>Telepon: {{ $data['pelanggan']->telepon }}</div>
                        </div>
                    </td>
                @endforeach
