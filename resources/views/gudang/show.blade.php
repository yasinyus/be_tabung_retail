...
<head>
    ...
    <title>Detail Gudang - {{ $gudang->kode_gudang }}</title>
...
<body class="bg-gradient-to-br from-green-50 to-emerald-100 min-h-screen">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Detail Gudang</h1>
            <p class="text-gray-600">Informasi Gudang PT GAS Tabung Retail</p>
        </div>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-8 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold mb-2">{{ $gudang->kode_gudang }}</h2>
                            <p class="text-green-100">{{ $gudang->nama_gudang }}</p>
                        </div>
                        <div class="text-right">
                            <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                                <div class="text-sm text-green-100">ID Gudang</div>
                                <div class="text-lg font-bold">#{{ str_pad($gudang->id, 4, '0', STR_PAD_LEFT) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between py-3 border-b border-gray-100">
                                    ...
                                    <span class="font-semibold px-3 py-1 rounded-full
                                        @if($gudang->tahun_gudang < date('Y') - 30) bg-red-50 text-red-700
                                        @elseif($gudang->tahun_gudang < date('Y') - 20) bg-yellow-50 text-yellow-700
                                        @else bg-green-50 text-green-700
                                        @endif">
                                        {{ $gudang->tahun_gudang }}
                                        @if($gudang->tahun_gudang < date('Y') - 30)
                                            <i class="fas fa-exclamation-triangle ml-1 text-red-500"></i>
                                        @elseif($gudang->tahun_gudang < date('Y') - 20)
                                            <i class="fas fa-exclamation-circle ml-1 text-yellow-500"></i>
                                        @endif
                                    </span>
                                </div>

                                @if($gudang->keterangan)
                                <div class="py-3">
                            ...
                            <div class="text-center">
                                @if($gudang->qr_code)
                                    <div class="bg-white border-2 border-gray-200 rounded-xl p-6 inline-block">
                                        <img src="data:image/svg+xml;base64,{{ $gudang->getQrCodeBase64() }}" 
                                             alt="QR Code {{ $gudang->kode_gudang }}" 
                                             class="mx-auto"
                                             style="width: 200px; height: 200px;">
                                    </div>
                                    <p class="text-sm text-gray-500 mt-3">
                                        <i class="fas fa-mobile-alt mr-1"></i>
                                        Scan untuk melihat detail gudang
                                    </p>
                                @else
                ...
                <div class="bg-gray-50 px-6 py-4 border-t">
                    <div class="flex flex-col sm:flex-row justify-between items-center text-sm text-gray-600">
                        <div class="flex items-center mb-2 sm:mb-0">
                            ...
                            Dibuat: {{ $gudang->created_at->format('d F Y, H:i') }}
                        </div>
                        
                        @if($gudang->created_at != $gudang->updated_at)
                        <div class="flex items-center">
                            <i class="fas fa-edit text-gray-400 mr-2"></i>
                            Diperbarui: {{ $gudang->updated_at->format('d F Y, H:i') }}
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="mt-6 text-center space-x-4">
                ...
                <button onclick="window.print()" 
                        class="bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-300 ease-in-out transform hover:scale-105">
        ...
        <div class="text-center mt-12 text-gray-500">
            <p class="text-sm">© {{ date('Y') }} PT GAS Tabung Retail - Sistem Manajemen Gudang</p>
        </div>
    </div>
