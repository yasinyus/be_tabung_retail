...
# 3. Test dengan QR Code invalid
Write-Host "3. Testing dengan QR Code invalid..." -ForegroundColor Yellow
...
try {
    $invalidResponse = Invoke-RestMethod -Uri "$base_url/mobile/terima-tabung" -Method POST -Body $invalidTabungData -Headers $headers
    
    Write-Host "Response: $($invalidResponse | ConvertTo-Json -Depth 3)" -ForegroundColor Yellow
    
} catch {
    $errorDetails = $_.ErrorDetails.Message | ConvertFrom-Json
    if ($errorDetails.status -eq "error") {
        Write-Host "✓ Validasi QR Code berfungsi dengan baik!" -ForegroundColor Green
        Write-Host "Error Message: $($errorDetails.message)" -ForegroundColor Cyan
    } else {
...
Write-Host "- Gudang: GDG-001, GDG-002, GDG-999, dll (GDG-[3digit])"
Write-Host "- Armada: ARM-001, ARM-002, ARM-999, dll (ARM-[3digit])"
Write-Host "- Tabung: TBG-001, TBG-002, TBG-999, dll (TBG-[3digit])"
Write-Host ""
